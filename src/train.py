...
logfilename = f'log/{os.path.basename(__file__).split(".")[0]}_{ts}.log'
logging.basicConfig(filename=logfilename, format='%(asctime)s %(message)s', level=logging.INFO)

# -----------------------------------------------------------------------------
# EK: tuneable hyperparameters seem to be:
#     - eval_iters (?)
#     - block_size
#     - batch_size
...
if compile_model:
    ...
    compile_end = timer()
    logger.info(f'compilation time: {compile_end - compile_start:.2f}s')
...
training_end = timer()
logger.info(f'Training time: {training_end - training_start:0.2f}s')
